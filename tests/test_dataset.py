...
from yukarin_wavernn.dataset import BaseWaveDataset, Input, WavesDataset
        ...
        for sampling_rate, local_sampling_rate, sampling_length, time_length in [
            ...
            with self.subTest(
                ...
                for _ in range(10):
                    ...
                    wave_as_local = wave.reshape(int(sampling_length // scale), -1).min(
                        axis=1
                    )
        ...
        for (
            ...
            with self.subTest(
                ...
                for _ in range(10000):
                    ...
                    self.assertEqual(len(silence), sampling_length)
                    self.assertEqual(
                        len(local), (sampling_length + local_padding_size * 2) // scale
                    )

                    num_pad = numpy.isnan(local).sum()
                    self.assertLessEqual(num_pad, local_padding_size)

                    self.assertTrue(
                        not numpy.isnan(local[0]) or not numpy.isnan(local[-1])
                    )
                    ...
                    self.assertTrue(numpy.all(wave_as_local == local_wo_pad))
...
class TestWavesDataset(unittest.TestCase):
    ...
    def setUp(self):
        waves = [
            numpy.ones(self.num // 2) * -1,
            numpy.ones(self.num // 2),
        ]
        self.inputs = [
            Input(
                wave=Wave(wave=w, sampling_rate=self.sampling_rate),
                local=SamplingData(
                    array=numpy.empty((len(w), 0)), rate=self.sampling_rate
                ),
                silence=SamplingData(
                    array=numpy.zeros((len(w),), dtype=bool), rate=self.sampling_rate
                ),
            )
            for w in waves
        ]
    ...
    def test_get_single(self):
        for mulaw in [True, False]:
            with self.subTest(mulaw=mulaw):
                ...
                self.assertTrue(numpy.all(dataset[0]["coarse"] == -1))
                self.assertTrue(numpy.all(dataset[1]["coarse"] == 1))
